...
export default function QuestionForm({ lang, enabled }: { lang: Language; enabled: boolean }) {
  ...
  return <div className="question-panel"><p className="eyebrow">{ru ? "Есть вопрос?" : "Have a question?"}</p><h3>{ru ? "Напишите нам" : "Send us a note"}</h3>
    {enabled ? <form onSubmit={submit} noValidate>
      <label>{ru ? "Имя" : "Name"}<input ref={nameRef} name="name" autoComplete="name" maxLength={100} aria-invalid={!!error && !nameRef.current?.value} aria-describedby={error ? "question-error" : undefined} /></label>
      <label>{ru ? "E-mail или телефон" : "Email or phone"}<input ref={contactRef} name="contact" autoComplete="email" maxLength={150} aria-invalid={!!error && !contactRef.current?.value} aria-describedby={error ? "question-error" : undefined} /></label>
      <label>{ru ? "Ваш вопрос" : "Your question"}<textarea ref={questionRef} name="question" rows={5} maxLength={3000} aria-invalid={!!error && !questionRef.current?.value} aria-describedby={error ? "question-error" : undefined} /></label>
