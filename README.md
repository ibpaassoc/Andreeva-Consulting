...
4. Provide a verified mail sender, recipient, API key, and shared Upstash rate-limit store for the question form (`QUESTION_FROM_EMAIL`, `QUESTION_TO_EMAIL`, `RESEND_API_KEY`, `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN`). The API sends plain-text messages server-side; no keys are exposed to the browser. The rate limiter expects Vercel's trusted client-IP header and fails closed if its deployment or store configuration is missing.
